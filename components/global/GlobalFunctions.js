...
module.exports = {
  ...
  isUserCheatingWithServerDate: function(serverDate) {
    ...
    let isServerDateDifferent =
      serverDate && Math.abs(today.getTime() - serverDate.getTime()) > one_hour;
  ...
  calculateAppExpirationState: function() {
    // serverDate is a verified date from our servers
    let today = new Date();
    let startDate = this.dates().startDate;
    let endDate = this.dates().endDate;
  ...
  overrideActions: function() {
    //override the app inactivity (used for trying app / using after expiration)
    return {
      openApp: "OPENAPP",
      tryApp: "TRYAPP",
      logout: "LOGOUT"
  ...
  asyncUpdateServerProfile: async function(
    id,
    newProfile,
    shouldUseDummyData,
    token,
    ...
    let url = "https://jumbosmash2017.herokuapp.com/profile/update/"
      .concat(id)
      .concat("/")
      .concat(token);
    fetch(url, {
      ...
      .then(response => {
        if (this.isGoodResponse(response)) {
          ...
          Alert.alert("Success!", "Successfully updated your profile", options);
        } else {
          Alert.alert(
            "Error",
