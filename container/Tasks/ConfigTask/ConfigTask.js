...
class ConfigTask extends Component {
  ...
  componentDidMount() {
    ...
    if (task !== false) this.initTask(task);
      ...
      const checkExistCategory = this.props.categories.filter(
        cate => cate.name === this.state.task.category
      );
  ...
  initTask = id => {
    ...
    this.props.onInitTask(id, task => {
      ...
      if (+task.repeat === parseInt(task.repeat, 10)) {
        ...
        otherOption = `${
          translations.other
        } (${+repeatValue} ${convertNumberToDate(+selectedTime)})`;
      }

      const checkExistCategory = categories.filter(
        cate => cate.name === task.category
      );
            ...
            {/* <View style={styles.container}>
              ...
              <DatePicker
                ...
                iconComponent={
                  task.date ? (
                    <IconToggle
                      onPress={() => this.updateTask("date", "")}
                      name="clear"
                    />
                  ) : (
                    <IconToggle
                      onPress={() => this.datepickerDate.onPressDate()}
                      name="event"
                    />
                  )
                }
                ...
                onDateChange={date =>
                  this.updateTask("date", this.convertDate(date))
                }
              />
              {task.date !== "" && (
                <React.Fragment>
                  <DatePicker
                    ...
                    customStyles={{
                      ...
                      dateText: {
                        color:
                          +date < +now ? theme.overdueColor : theme.textColor
                      }
                    }}
                    onDateChange={date => {
                      this.updateTask(
                        "date",
                        `${task.date.slice(0, 10)} - ${date}`
                      );
                    }}
                  />
                  <Checkbox
              ...
              <View style={styles.selectCategory}>
                ...
                <IconToggle
                  onPress={() => this.toggleConfigCategory()}
                  name="playlist-add"
                />
              </View>
              <Subheader
              ...
              <View style={styles.picker}>
                ...
                >
                  ...
                  <Picker.Item
                    label={translations.priorityMedium}
                    value="medium"
                  />
