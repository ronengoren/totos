...
export const setLocalNotification = async (task, color) => {
  ...
  if (notificationStatus === "granted") {
    ...
    if (task.notification_id !== null)
      await deleteLocalNotification(task.notification_id);
...
export const setCalendarEvent = async (task, color, calendarId = null) => {
  ...
  if (status === "granted" && Platform.OS !== "ios") {
    ...
    for (let i = 0; i < calendars.length; i++) {
      if (
        calendars[i].ownerAccount === "Maker" &&
        calendars[i].allowsModifications
      ) {
    ...
    if (!calendarId) {
      ...
      const details = {
        ...
        allowedAttendeeTypes: [
          Calendar.AttendeeType.REQUIRED,
          Calendar.AttendeeType.NONE
        ],
  ...
  if (calendarId !== null) {
    ...
    if (task.event_id) {
      ...
      return await Calendar.updateEventAsync(task.event_id + "", detailsEvent, {
        futureEvent: true
      });
    } else {
      // Create new event
...
export const deleteCalendarEvent = async event_id => {
  await Calendar.deleteEventAsync(event_id + "", { futureEvent: true }).catch(
    err => err
  );
};
