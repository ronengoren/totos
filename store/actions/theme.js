...
export const setSelectedTheme = id => {
  return dispatch => {
    db.transaction(
      tx => {
        tx.executeSql(
          "update settings set theme = ? where id = 0;",
          [id],
          () => {
            dispatch(initTheme());
          }
  ...
  return dispatch => {
    if (theme.id) {
      db.transaction(
        tx => {
          tx.executeSql(
            "UPDATE themes SET name = ?, primaryColor = ?, primaryBackgroundColor = ?, secondaryBackgroundColor = ?, textColor = ?, headerTextColor = ?, bottomNavigationColor = ?, actionButtonColor = ?, actionButtonIconColor = ?, overdueColor = ?, doneButtonColor = ?, doneButtonTextColor = ?, undoButtonColor = ?, undoButtonTextColor = ?, noneColor = ?, noneTextColor = ?, lowColor = ?, lowTextColor = ?, mediumColor = ?, mediumTextColor = ?, highColor = ?, highTextColor = ? WHERE id = ?;",
            [
              theme.name,
              theme.primaryColor,
              ...
              theme.bottomNavigationColor,
              theme.actionButtonColor,
              theme.actionButtonIconColor,
              theme.overdueColor,
      ...
      db.transaction(
        tx => {
          tx.executeSql(
            "INSERT INTO themes (name, primaryColor, primaryBackgroundColor, secondaryBackgroundColor, textColor, headerTextColor, bottomNavigationColor, actionButtonColor, actionButtonIconColor, overdueColor, doneButtonColor, doneButtonTextColor, undoButtonColor, undoButtonTextColor, noneColor, noneTextColor, lowColor, lowTextColor, mediumColor, mediumTextColor, highColor, highTextColor) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);",
            [
              theme.name,
              theme.primaryColor,
              theme.primaryBackgroundColor,
